...
def build_evidence_bundle(*, context: V3Context, vector: FeatureVectorV3, event_id: str, model_version: str, prediction: float) -> dict:
    return {
        "event_id": event_id,
        ...
        "price_context": {name: vector.values[name] for name in vector.values if name.startswith("return_") or name.startswith("realized_volatility_") or name.startswith("distance_from_")},
        "company_history_summary": {
            name: vector.values[name]
            for name in vector.values
            if name.startswith("prior_earnings") or name.startswith("mean_prior_") or name.startswith("similar_")
        },
        "peer_summary": {
            name: vector.values[name]
            for name in vector.values
...
def persist_evidence_bundle(*, context: V3Context, vector: FeatureVectorV3, event_id: str, model_version: str, prediction: float, directory: str | Path | None = None) -> Path:
    ...
    bundle = build_evidence_bundle(context=context, vector=vector, event_id=str(event_id), model_version=model_version, prediction=prediction)
    try:
