...
def test_candidate_keeps_normal_promotion_false_and_uses_operator_metadata(tmp_path):
    ...
    assert model.disclosure_parser_version == PARSER_VERSION
...
def test_candidate_rejects_stale_parser_version(tmp_path):
    payload = _artifact()
    payload["disclosure_parser_version"] = "stale"
    with pytest.raises(ValueError, match="disclosure parser"):
        V3LiteCandidateModel(_write(tmp_path, payload))
